...
A classe \texttt{InstanceLoader} implementa essa preparação. O método \texttt{parsePmedFile} retorna o tamanho $n$, o parâmetro $k$ e a matriz de distâncias resultante.
...
A implementação encontra-se em \texttt{ApproximateKCenters.java}. O algoritmo executa em tempo $O(k \cdot n^2)$, dominado pelo cálculo repetido de distâncias mínimas aos centros atuais.
...
A classe \texttt{Experiments} automatiza a coleta de resultados, gravando-os em \texttt{results/resultados.csv}.
...
tgc-cc-tp02-k-centers/
  ...
  src/main/java/br/puc/tgc/kcenters/
    ...
    ExactKCenters.java       # Algoritmo exato (ILP com ojAlgo + busca binaria)
    ApproximateKCenters.java # Algoritmo de Gonzalez
    DownloadData.java        # Download das instancias
    Experiments.java         # Execucao dos experimentos
    GeneratePlots.java       # Geracao de graficos
  results/
